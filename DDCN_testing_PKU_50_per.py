    ...
    def infinite_batch(self):
        ...
        for i in range(0, len(interval_index)):
            ...
            for k in range(0, len(interval_index[i])):
                if (k % 2 == 1):
                    length = int((interval_index[i][k] - interval_index[i][k - 1]) / 2)
                    non_aberved.append([interval_index[i][k] - length, interval_index[i][k]])
            non_oberved_array.append(non_aberved)

        for i in range(0, len(labels_50_per)):
            for j in range(0, len(non_oberved_array[i])):
                labels_50_per[i][non_oberved_array[i][j][0]:non_oberved_array[i][j][1]] = -1

        while True:
            for d, l in zip(dataset, labels_50_per):

                yield np.array(d).reshape([1, len(d), 150]), np.array(l)
